...
STATE_FILE = TEMP_DIR / "gs1_project_state.json"
...
LOCALIZER = CODE_DIR / "compiler" / "gs1_localizer.py"
VERIFIER = CODE_DIR / "verifier" / "gs1_verifier.py"
...
OUTPUT_ROM = TEMP_DIR / "golden_sun_1_ukr.gba"
STRUCTURE_REPORT = TEMP_DIR / "gs1_structure_report.json"
...
def required_files_ok(verbose: bool = True) -> bool:
    ...
    if verbose:
        if missing:
            ...
            print("All required GS1 files/resources are present.")
    return not missing
...
def generate_translation_json() -> None:
    ...
    if not required_files_ok(verbose=False):
        ...
        return
    print("[INFO] Generating translation template from the clean GS1 ROM...")
...
def latest_final_rom() -> Path | None:
    if not FINAL_OUTPUT_DIR.exists():
        return None
    roms = sorted(FINAL_OUTPUT_DIR.glob("Golden_Sun_1_UA_*.gba"), key=lambda path: path.stat().st_mtime)
...
def build_final_rom() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="final_build_", dir=TEMP_DIR) as tmp_name:
        ...
        final_rom = FINAL_OUTPUT_DIR / f"Golden_Sun_1_UA_{final_datetime_mark()}.gba"
...
def suggested_next_action() -> str:
    if not required_files_ok(verbose=False):
        return "Run option 1 and add missing GS1 files/resources."
    if not TRANSLATION_JSON.exists():
...
def print_header() -> None:
    print("\nGolden Sun 1 Localization Helper")
    print("=" * 34)
    print("This helper builds a localized Golden Sun 1 ROM.")
    print("Required user-provided file in GS1/input/:")
    print("  - golden_sun_1_eng.gba  (clean English GS1 ROM)")
    print("\nBundled Cyrillic font and hook resources are stored in GS1/code/resources/.")
...
def menu() -> None:
    while True:
        print("\nMenu:")
        print("  1. Check required GS1 files/resources")
